    ...
    def run_lap(self, driver_name: str, lap_name: str, aggressiveness: int):
        ...
        if random.random() < crash_chance:
            result = {
                ...
                "time_str": "DNF",
                "msg": f"CRASH: {driver_name} lost control at aggressiveness {aggressiveness} and hit the wall."
            }
...
def render_miami_track(active_dur, last_dur, other_dur, driver_name, key_suffix=""):
    ...
    def get_ghost_svg(dur, color):
        if not dur: return ""
        return f'<circle r="6" fill="{color}" opacity="0.6"><animateMotion dur="{dur}s" repeatCount="1" path="{track_path}" rotate="auto" fill="freeze"/></circle>'
    ...
    <div style="background: {bg_color}; padding: 20px; border-radius: 15px; border: 2px solid #333; text-align: center; color: white; font-family: sans-serif;">
        ...
        <svg viewBox="0 0 500 250" xmlns="http://www.w3.org/2000/svg" style="width: 100%; max-width: 600px;">
            ...
            {get_ghost_svg(last_dur, last_lap_color)}
            {get_ghost_svg(other_dur, other_driver_color)}
            <circle r="9" fill="{active_color}">
                <animateMotion dur="{active_dur}s" repeatCount="1" path="{track_path}" rotate="auto" fill="freeze"/>
            </circle>
        </svg>
        <div style="margin-top: 10px; font-family: monospace; font-size: 1.2em; color: {active_color};">
            <span id="timer_{key_suffix}">0.000</span>s
        </div>
    </div>
    <script>
        (function() {{
            ...
            let timerDisplay = document.getElementById('timer_{key_suffix}');
...
for idx, msg in enumerate(st.session_state.messages):
    ...
    elif msg.get("type") == "map":
        with st.expander(f"🗺️ View Lap: {msg['driver']} - {msg['lap_name']} ({msg['time_str']})", expanded=True):
            cols = st.columns([4, 1])
            with cols[0]:
                render_miami_track(
                    active_dur=msg["active_dur"],
                    last_dur=msg["last_dur"],
                    other_dur=msg["other_dur"],
                    driver_name=msg["driver"],
                    key_suffix=str(idx)
                )
            with cols[1]:
...
# Detect if we just added a user message and need to respond
if st.session_state.messages[-1]["role"] == "user":
    last_prompt = st.session_state.messages[-1]["content"]
    new_msg = types.Content(role="user", parts=[types.Part(text=last_prompt)])
            ...
            for event in events:
                ...
                if st.session_state.shared_ctx["last_lap"]:
                    ...
                    st.session_state.messages.append({
                        ...
                        "type": "map",
                        "driver": anim["driver"],
                        "lap_name": anim["lap_name"],
                        "time_str": anim["time_str"],
                        "active_dur": anim["duration_seconds"],
                        "last_dur": anim["last_lap_seconds"],
                        "other_dur": anim["other_driver_latest"]
                    })
                    # Add result as a text system message too
                    status_icon = "🏁" if anim["status"] == "SUCCESS" else "⚠️"
                    st.session_state.messages.append({
                        "role": "system",
                        "type": "system",
                        "content": f"{status_icon} {anim['msg']}"
                    })
